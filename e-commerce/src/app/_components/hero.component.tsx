...
import SpotlightCard from "@/components/spotlight-card.component";
import { ArrowRight } from "lucide-react";

// Social Media Icons Constants
const SOCIAL_ICONS = [
  {
    name: 'facebook',
      ...
      <div className="container-lg mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
          <div className="space-y-8 text-white relative">
            {/* Social Media Icons - Reusable and clean */}
            <div className="hidden lg:flex flex-col space-y-3 absolute top-0 left-0">
              {SOCIAL_ICONS.map((icon, index) => (
                <div 
                  key={icon.name}
                  className="w-8 h-8 bg-white/10 backdrop-blur rounded-full flex items-center justify-center hover:bg-white/20 transition-colors cursor-pointer"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
            ...
            {/* Main Content - Increased left margin for wider gap from social icons */}
            <div className="max-w-[20rem] ml-20">
              <ShinyText
            ...
            {/* Dots Navigation - Adjusted position to match new content layout */}
          ...
          <div className="relative hidden lg:flex justify-end">
            <div className="flex gap-3 items-start">
              <div className="flex flex-col gap-3 -mt-2">
                <SpotlightCard
                  className="relative overflow-hidden shadow-xl transform hover:scale-105 transition-transform duration-300 !p-0 !border-0 !bg-transparent"
                  spotlightColor="rgba(125, 184, 0, 0.3)"
                  rounded={false}
                >
                  <img
                    src="/hero-1.png"
              ...
              <div className="mt-4">
                <SpotlightCard
                  className="relative overflow-hidden shadow-2xl transform hover:scale-105 transition-transform duration-300 !p-0 !border-0 !bg-transparent"
                  spotlightColor="rgba(125, 184, 0, 0.3)"
                  rounded={false}
                >
                  <img
